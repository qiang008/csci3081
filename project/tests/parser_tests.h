// Copyright 2017 Liyu Qiang, Kyle Doerrler
...
class ParserTestSuite : public CxxTest::TestSuite {
 public:
  ...
  void test_setup_code() {
    p = new fcal::parser::Parser();
    TS_ASSERT(p);
  }

  void test_parse_bad_syntax() {
    const char *text =
      fcal::scanner::ReadInputFromFile("./samples/bad_syntax_good_tokens.dsl");
    TS_ASSERT(text);
    fcal::parser::ParseResult pr = p->Parse(text);
    TS_ASSERT(!pr.ok());
  }

  void test_parse_sample_1() {
    const char *filename = "./samples/sample_1.dsl";
    const char *text = fcal::scanner::ReadInputFromFile(filename);
    TS_ASSERT(text);
    fcal::parser::ParseResult pr = p->Parse(text);
